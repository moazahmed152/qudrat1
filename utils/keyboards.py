...
def chapters_keyboard(chapters):
    rows = [
        [InlineKeyboardButton(ch["chapter_name"], callback_data=f"chapter:{ch['chapter_id']}")]
        for ch in chapters
    ]
...
def lessons_keyboard(chapter_id, lessons):
    rows = [
        [InlineKeyboardButton(ls["lesson_name"], callback_data=f"lesson:{chapter_id}:{ls['lesson_id']}")]
        for ls in lessons
    ]
    return InlineKeyboardMarkup(rows)
...
def rule_content_keyboard(chapter_id, lesson_id, rule_id, num_examples=10, completed=None):
    ...
    for i in range(1, num_examples + 1):
        ...
        rows.append([
            InlineKeyboardButton(label, callback_data=f"example:{chapter_id}:{lesson_id}:{rule_id}:{i}")
        ])
...
def t_chapters_keyboard(chapters):
    rows = [
        [InlineKeyboardButton(ch["chapter_name"], callback_data=f"tchapter:{ch['chapter_id']}")]
        for ch in chapters
    ]
    return InlineKeyboardMarkup(rows)
...
def t_lessons_keyboard(chapter_id, lessons):
    rows = [
        [InlineKeyboardButton(ls["lesson_name"], callback_data=f"tlesson:{chapter_id}:{ls['lesson_id']}")]
        for ls in lessons
    ]
    return InlineKeyboardMarkup(rows)
...
def t_rules_keyboard(chapter_id, lesson_id, rules):
    rows = [
        [InlineKeyboardButton(r["rule_name"], callback_data=f"trule:{chapter_id}:{lesson_id}:{r['rule_id']}")]
        for r in rules
    ]
    return InlineKeyboardMarkup(rows)
...
def t_question_keyboard(qid, options):
    rows = [
        [InlineKeyboardButton(opt, callback_data=f"tans:{qid}:{i}")]
